--[[
    ...
    Features: PVT + COPY + PROFILE popup, 3 new god-tier auras, scroll lock, creator join alert
]]
...
local function showMessagePopup(sender, text, userId, shownName)
    ...
    if isOwnMessage then
        pvtBtn_disabled = true -- placeholder, PVT hidden for own messages
    end
...
local function createAuraForPlayer(targetPlayer, auraType)
    ...
    for _, data in ipairs(structureToUse) do
        ...
        p.Parent = model
        table.insert(createdParts, {part = p, offset = data.offset or CFrame.new(), isDetail = data.isDetail})
    end
            ...
            for i, item in ipairs(data.Parts) do
                if item.part and item.part.Parent then
                    ...
                    if data.Type == "orbit" then
                        ...
                        elseif data.Type == "void_wings" then rotSpeed = 0.8
                        elseif data.Type == "inferno_king" then rotSpeed = 1.5
                        elseif data.Type == "cosmic_god" then rotSpeed = 1.0
                        elseif data.Type == "mecha" then rotSpeed = 0
                        end
    ...
    if cmd == "!aura" then
        ...
        if targetUsername ~= "" then
            if not AuraRegistry[selectedAura] then
                addMessageToUI(nil, "⚡ [SYSTEM] : Invalid aura! Use: mecha, orbit, crown, shield, halo, spikes, matrix, void_wings, inferno_king, cosmic_god.", nil, true)
                return
            end
